...
const AdminDashboardScreen: React.FC = () => {
  ...
  const { feedbacks, getAllFeedbacks, updateFeedbackStatus } =
    useFeedbackContext();

  const [newCourse, setNewCourse] = useState<Partial<Course>>({});
  const [showCourseForm, setShowCourseForm] = useState(false);
  const [currentView, setCurrentView] = useState<
    "courses" | "feedbacks" | "users"
  >("courses");
  ...
  const handleCreateCourse = async () => {
    ...
    alert(`The course ${newCourse.title} has been created successfully`);
  };
            ...
            {feedbacks.map((feedback) => (
              <View key={feedback._id} style={styles.itemContainer}>
                ...
                <CustomButton
                  title="Review"
                  variant="primary"
                  handlePress={() => handleReviewFeedback(feedback._id!!)}
                />
              </View>
            ))}
          </>
          ...
          <ScrollView style={styles.container}>
            ...
            {showCourseForm ? (
              <View style={styles.formContainer}>
                ...
                />
                <View style={{ display: "flex", gap: 20, justifyContent: "space-between", flexDirection: "row" }}>
                  <CustomButton
                    title="Cancel"
                    variant="secondary"
                    handlePress={() => setShowCourseForm(false)}
                  />
                  <CustomButton
                    title="Submit"
                    variant="primary"
              ...
              <>
                ...
                <View style={styles.cardContainer}>
                  ...
                  <Card
                    ...
                    action={() => setCurrentView("feedbacks")}
                  />
                  <Card
                    title="Users"
                    value={users?.length}
