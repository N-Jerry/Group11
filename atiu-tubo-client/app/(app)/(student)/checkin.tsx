...
const CheckInScreen = () => {
  ...
  const checkBiometrics = async () => {
    try {
      ...
      if (!hasHardware) {
        alert(
          "Biometric authentication is not available: Your device does not support biometric authentication."
        );
      } else if (!isEnrolled) {
        alert(
          "Biometric authentication is not set up: Please set up biometric authentication in your device settings."
        );
      } else {
      ...
      alert(
        "Biometrics check error: Could not check biometric availability. Please try again."
      );
    }
  };
  ...
  const authenticateWithBiometrics = async () => {
    if (!biometricsAvailable) {
      alert(
        "Biometric authentication not available: Please make sure biometric authentication is set up on your device."
      );
      return;
    }
  ...
  const handleMarkAttendance = () => {
    if (typeof sessionId === "string" && user?._id) {
      markAttendance(sessionId, user._id, "present")
        .then(() => {setConfirmed(true); router.push("/student") })
        .catch((error) => {
          console.error(error);
              ...
              {confirmed ? (
                ...
                <View style={styles.checkInContainer}>
                  ...
                  <Text style={styles.instruction}>
                    Place your finger on your fingerprint sensor to mark
                    attendance
                  </Text>
                  <Text>Hello</Text>
                  ...
                  <Button
                    title="Authenticate and Mark Attendance"
